...
if [ -z "$DIR" ]; then
    ...
    sed -i "s|dspace.dir.*=.*|dspace.dir=${DIR}|1" /opt/dspace/dspace/config/dspace.cfg
fi
...
if [ -z "$HOSTNAME" ]; then
    ...
    sed -i "s|dspace.hostname.*=.*|dspace.hostname=${HOSTNAME}|1" /opt/dspace/dspace/config/dspace.cfg
fi
